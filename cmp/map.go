// Rewrite from go@1.20.3 sync.Map
...
//
// In the terminology of the Go memory model, MapOf arranges that a write operation
...
type MapOf[K comparable, V comparable] struct {
	mu sync.Mutex
	...
	misses int
	// marks this entry as having been deleted
	expunged *V
}
...
func NewMapOf[K comparable, V comparable]() *MapOf[K, V] {
	return &MapOf[K, V]{
		expunged: new(V),
	}
}

func newEntry[V comparable](i V, expunged *V) *entry[V] {
	e := &entry[V]{
		expunged: expunged,
	}
	e.p.Store(&i)
...
func (m *MapOf[K, V]) loadReadOnly() readOnly[K, V] {
...
func (m *MapOf[K, V]) LoadOrStore(key K, value V) (actual V, loaded bool) {
	...
	if e, ok := read.m[key]; ok {
		...
		m.dirty[key] = newEntry[V](value, m.expunged)
...
func (m *MapOf[K, V]) CompareAndSwap(key K, old, new V) bool {
	read := m.loadReadOnly()
	if e, ok := read.m[key]; ok {
