...
func applyMap(calls []mapCall) ([]mapResult, map[any]any) {
	return applyCalls(cmp.NewMapOf[any, any](), calls)
}
...
func TestIssue40999(t *testing.T) {
	var m = cmp.NewMapOf[interface{}, struct{}]()
...
func TestMapRangeNestedCall(t *testing.T) { // Issue 46399
	...
	m.Range(func(key int, value string) bool {
		m.Range(func(key int, value string) bool {
			...
			// 42 to the MapOf. In this case, the key 42 and associated value should be
			// removed from the MapOf. Therefore any future range won't observe key 42
		...
		// Remove key from MapOf on-the-fly.
		m.Delete(key)
		return true
	})
	...
	if length != 0 {
		t.Fatalf("Unexpected cmp.MapOfOf size, got %v want %v", length, 0)
	}
}
